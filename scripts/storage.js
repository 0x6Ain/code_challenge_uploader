...
chrome.storage.local.get("isSync", (data) => {
  ...
  if (!data || !data.isSync) {
    ...
    chrome.storage.local.set({ isSync: true }, (data) => {
      console.log("Synced to local values");
    });
  } else {
...
async function getToken() {
  return await getObjectFromLocalStorage(STORAGE_KEYS.TOKEN);
}

async function getGithubUsername() {
  return await getObjectFromLocalStorage(STORAGE_KEYS.USERNAME);
}

async function getStats() {
  return await getObjectFromLocalStorage(STORAGE_KEYS.STATS);
}

async function getHook() {
  return await getObjectFromLocalStorage(STORAGE_KEYS.HOOK);
}
...
async function getModeType() {
  return await getObjectFromLocalStorage(STORAGE_KEYS.MODE);
}

async function saveToken(token) {
  return await saveObjectInLocalStorage({ [STORAGE_KEYS.TOKEN]: token });
}
